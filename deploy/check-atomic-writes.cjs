...
const CRITICAL = [
  ...
  { id: "full-reboot-reason.json", files: ["spirit.bio.organs/hands.executes/executes.ts"],
    match: /writeFileSync\(\s*join\(rcDir, "full-reboot-reason\.json"\)/, why: "重启后 heart JSON.parse 读取", atomicOk: /writeFileAtomic\(\s*join\(rcDir, "full-reboot-reason\.json"\)/ },
