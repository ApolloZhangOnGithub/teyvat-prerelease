...
// diff 行内高亮（2026-09-11）：变化部分用更亮的背景色而非 bold+underline（参考 Claude Code 风格）
        ...
        renderResult: (result, _opts, t, ctx) => {
            ...
            for (const l of lines) {
                ...
                if (l.startsWith("+")) {
                    ...
                    // 行内 diff：如果前面有匹配的 - 行，高亮变化部分（双向：- 行也高亮）
        ...
        if (this.hasRendererDefinition()) {
            ...
            if (!callRenderer) {
                ...
                try {
                    ...
                    } catch (e) { console.error("[god.tui/overrides/modes/interactive/components/tool-execution.js] " + (e?.message || e)); }
