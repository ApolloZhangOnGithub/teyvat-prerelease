...
function inflateSnapshots(messages) {
    ...
    for (const msg of messages) {
        if (msg && msg.customType === "memory-snapshot" && typeof msg.content === "string" && msg.content.startsWith("__gz__")) {
            try { msg.content = gunzipSync(Buffer.from(msg.content.slice(6), "base64")).toString("utf8"); } catch (e) { console.error("[god.tui/overrides/pi-dist/core/agent-session.js] " + (e?.message || e)); }
        }
    }
}
export class AgentSession {
    ...
    _installAgentToolHooks() {
        ...
        this.agent.afterToolCall = async ({ toolCall, args, result, isError }) => {
            const runner = this._extensionRunner;
            // teyvat: pi ignores terminate:true in tool results.
            // We must abort the agent loop ourselves to prevent the model
            // from getting another turn after hibernate/wait.
            // ⚠️ ISSUE 116（2026-08-18）：原 queueMicrotask 异步 abort 与用户消息投递竞态——
            // 打断 wait 时新轮被误杀（agent_start 卡 27s）或中断通知破坏前缀缓存（prefill 20s，
            // ISSUE 115 实测）。abortFn 仅取消 signal + 状态清理，同步调用安全且消除竞态窗口。
            if (result.terminate) {
                try { runner.abortFn?.(); } catch (e) { console.error("[god.tui/overrides/pi-dist/core/agent-session.js] " + (e?.message || e)); }
            }
    ...
    _handleAgentEvent = async (event) => {
        ...
        if (event.type === "message_end") {
            // Decompress gzipped snapshot content before agent sees it
            if (event.message?.customType === "memory-snapshot" && typeof event.message.content === "string" && event.message.content.startsWith("__gz__")) {
                
                try { event.message.content = gunzipSync(Buffer.from(event.message.content.slice(6), "base64")).toString("utf8"); } catch (e) { console.error("[god.tui/overrides/pi-dist/core/agent-session.js] " + (e?.message || e)); }
            }
            // Check if this is a custom message from extensions
            if (event.message.role === "custom") {
                // Persist as CustomMessageEntry
                let content = event.message.content;
                if (event.message.customType === "memory-snapshot" && typeof content === "string" && content.length > 1000) {
                    
                    try { content = "__gz__" + gzipSync(content).toString("base64"); } catch (e) { console.error("[god.tui/overrides/pi-dist/core/agent-session.js] " + (e?.message || e)); }
                }
                this.sessionManager.appendCustomMessageEntry(event.message.customType, content, event.message.display, event.message.details);
            }
    ...
    dispose() {
        ...
        this._extensionRunner.invalidate("This extension ctx is stale after session replacement or reload. Do not use a captured genshin or command ctx after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload(). For newSession, fork, and switchSession, move post-replacement work into withSession and use the ctx passed to withSession. For reload, do not use the old ctx after await ctx.reload().");
    ...
    async sendCustomMessage(message, options) {
        ...
        if (options?.deliverAs === "nextTurn") {
        ...
        else if (options?.deliverAs === "interrupt") {
            // teyvat 2026-08-14：interrupt 投递 = 强制切断（与用户输入同级：先 abort 当前 run 再立即注入）。
            // 复用缓冲管线但不等 50ms 批延迟：先 abort，30ms 后 flush（_flushBatch 在 run 未停时自会重试）。
            this._pendingBatchMessages.push(appMessage);
            clearTimeout(this._batchTimer);
            if (this._isAgentRunActive) {
                try { this.abort().catch((e) => console.error("[god.tui/overrides/pi-dist/core/agent-session.js] interrupt: abort() rejected: " + (e?.message || e))); } catch (e) { console.error("[god.tui/overrides/pi-dist/core/agent-session.js] " + (e?.message || e)); }
            }
            ...
            // 缓冲管线：收集并发到达的消息，50ms 后统一合并注入
            this._pendingBatchMessages.push(appMessage);
            clearTimeout(this._batchTimer);
            this._batchTimer = setTimeout(() => this._flushBatch(), 50);
        }
        else {
            this.agent.state.messages.push(appMessage);
            let content = message.content;
            if (message.customType === "memory-snapshot" && typeof content === "string" && content.length > 1000) {
                
                try { content = "__gz__" + gzipSync(content).toString("base64"); } catch (e) { console.error("[god.tui/overrides/pi-dist/core/agent-session.js] " + (e?.message || e)); }
            }
            this.sessionManager.appendCustomMessageEntry(message.customType, content, message.display, message.details);
            this._emit({ type: "message_start", message: appMessage });
            this._emit({ type: "message_end", message: appMessage });
        }
    }
    /** Flush batched triggerTurn messages into a single _runAgentPrompt */
    async _flushBatch() {
        const messages = this._pendingBatchMessages.splice(0);
        if (messages.length === 0) return;
        if (this._isAgentRunActive) {
            // agent 还在运行：放回队列延迟重试，绝不丢消息。
            // （后台命令完成等 triggerTurn 消息若恰在 turn 收尾窗口到达，直接丢弃会让结果永远消失）
            this._pendingBatchMessages.push(...messages);
            clearTimeout(this._batchTimer);
            this._batchTimer = setTimeout(() => this._flushBatch(), 200);
            return;
        }
        await this._runAgentPrompt(messages.length === 1 ? messages[0] : messages);
    }
    /**
    ...
    async setModel(model) {
        ...
        if (previousModel && !modelsAreEqual(previousModel, model)) {
          ...
          catch (e) { console.error("[god.tui/overrides/pi-dist/core/agent-session.js] model-switch notice: " + (e?.message || e)); }
        }
    }
    /**
    ...
    async _cycleScopedModel(direction) {
        ...
        // 2026-08-18 per-agent：写本 agent registry 记录（不写共享 settings）
        savePerAgentModel(next.model.provider, next.model.id);
    ...
    async _cycleAvailableModel(direction) {
        ...
        // 2026-08-18 per-agent：写本 agent registry 记录（不写共享 settings）
        savePerAgentModel(nextModel.provider, nextModel.id);
        // Re-clamp thinking level for new model's capabilities
        this.setThinkingLevel(thinkingLevel);
    ...
    async compact(customInstructions) {
        ...
        try {
            ...
            if (!preparation) {
                // Session too small or already compacted — silently skip, don't error to user
                return undefined;
            }
            ...
            // Decompress gzipped snapshot refs
            for (const msg of this.agent.state.messages) {
                if (msg.customType === "memory-snapshot" && typeof msg.content === "string" && msg.content.startsWith("__gz__")) {
