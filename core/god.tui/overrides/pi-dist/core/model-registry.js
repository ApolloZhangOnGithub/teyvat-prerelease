 ...
 *
 * teyvat override（ISSUE 144 / 254）。注意：readFileSync 已由下方
 * `import { existsSync, readFileSync } from "fs"` 提供（上游原有行）——
 * 勿在顶部再加 `node:fs` 的同名导入：2026-09-14 的重复声明曾让整包 ESM
 * 加载直接 SyntaxError（Identifier 'readFileSync' has already been declared）。
 */
import {
    getModels,
    getProviders,
...
export class ModelRegistry {
    ...
    applyConfiguredModelOverride(providerName, model) {
        const modelOverride = this.getConfiguredModelOverride(
            providerName,
            model.id,
        );
    ...
    mergeCustomModels(builtInModels, customModels) {
        ...
        for (const customModel of customModels) {
            const existingIndex = merged.findIndex(
                (m) =>
                    m.provider === customModel.provider &&
                    m.id === customModel.id,
            );
            if (existingIndex >= 0) {
        ...
        try {
            ...
            for (const [providerName, providerConfig] of Object.entries(
                ...
                if (providerConfig.oauth === "radius") {
                    ...
                    registerCustomRadiusOAuthProvider(
                        providerName,
                        providerConfig.name,
    ...
    validateConfig(config) {
        ...
        for (const [providerName, providerConfig] of Object.entries(
            ...
            if (providerConfig.oauth && !providerConfig.baseUrl) {
                throw new Error(
                    `Provider ${providerName}: "baseUrl" is required when "oauth" is set.`,
                );
            }
    ...
    parseModels(config) {
        ...
        // 2026-09-14（用户）：models.dev catalog 的 input 补齐——teyvat 缓存里有各模型的 modalities.input
        // （GLM-5.3-flash 原生多模态等），models.json 条目未声明 input 时从这里补，否则 eyes 的
        // isVisionModel 拿不到 image 标记会拒绝注入（GLM 协议适配缺口，ISSUE 252 同期发现）。
        let _mdCatalogCache;
        const modelsDevInput = (providerName, modelId) => {
            try {
                if (_mdCatalogCache === undefined) {
                    const home =
                        process.env.PAIMON_HOME ||
                        `${process.env.HOME}/.teyvat`;
                    const agent = process.env.PAIMON_AGENT_ID || "";
                    _mdCatalogCache = JSON.parse(
                        readFileSync(
                            `${home}/RuntimeCache/${agent}/modelsdev-catalog.json`,
                            "utf8",
                        ),
                    );
                }
                const m = _mdCatalogCache?.[providerName]?.models?.[modelId];
                const inp = m?.modalities?.input;
                return Array.isArray(inp) && inp.length > 0 ? inp : undefined;
            } catch {
                return undefined;
            }
        };
        ...
        const getBuiltInDefaults = (providerName) => {
            ...
            const defaults = {
                api: builtIn[0].api,
                baseUrl: builtIn[0].baseUrl,
                contextWindow: builtIn[0].contextWindow,
                maxTokens: builtIn[0].maxTokens,
            };
        ...
        for (const [providerName, providerConfig] of Object.entries(
            ...
            for (const modelDef of modelDefs) {
                const api =
                    modelDef.api ?? providerConfig.api ?? builtInDefaults?.api;
                if (!api) continue;
                const baseUrl =
                    modelDef.baseUrl ??
                    providerConfig.baseUrl ??
                ...
                models.push({
                    ...
                    input: modelDef.input ??
                        modelsDevInput(providerName, modelDef.id) ?? ["text"],
                    cost: modelDef.cost ?? defaultCost,
                    contextWindow:
                        modelDef.contextWindow ??
                        builtInDefaults?.contextWindow ??
                        128000,
                    maxTokens:
                        modelDef.maxTokens ??
                        builtInDefaults?.maxTokens ??
                        16384,
                    headers: undefined,
                    compat,
                });
    ...
    async getApiKeyAndHeaders(model) {
        try {
            const providerConfig = this.providerRequestConfigs.get(
                model.provider,
            );
            ...
            return {
                ...
                headers:
                    headers && Object.keys(headers).length > 0
                        ...
                        : undefined,
                env:
                    providerEnv && Object.keys(providerEnv).length > 0
                        ? providerEnv
                        : undefined,
            };
        } catch (error) {
    ...
    async getApiKeyForProvider(provider) {
        ...
        return providerApiKey
            ? resolveConfigValueUncached(
                  providerApiKey,
                  this.authStorage.getProviderEnv(provider),
              )
            : undefined;
    }
    /**
