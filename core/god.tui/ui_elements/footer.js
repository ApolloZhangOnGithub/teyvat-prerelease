...
function getBirthTs(personId) {
    ...
    try {
        ...
        let ts = Date.now();
        try { const s = statSync(dir); ts = s.birthtimeMs || s.ctimeMs || ts; } catch (e) { console.error("[god.tui/ui_elements/footer.js] " + (e?.message || e)); }
        try { writeFileSync(birthPath, JSON.stringify({ birth_ts: ts, created: new Date(ts).toISOString(), source: ts < Date.now() - 60000 ? "dir_birthtime" : "first_run" })); } catch (e) { console.error("[god.tui/ui_elements/footer.js] " + (e?.message || e)); }
    ...
    render(width) {
        ...
        if (onlyContexted) {
            ...
            if (followPillW > 0 && middleSpace >= followPillW + 4) {
                ...
                const rightPad = middleSpace - followPillW - leftPad;
                line1 = modelLeft + " ".repeat(leftPad) + followPill + " ".repeat(rightPad) + rightLine1;
            } else {
        ...
        const sbInMessages = (globalThis.__genshinStatebarPosition ?? "footer") === "messages"; // 默认 footer（2026-09-22 统一）
