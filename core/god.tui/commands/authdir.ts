...
async function loadToolsAuth(pid: string): Promise<{ enabled: string[]; disabled: string[] }> {
  ...
  } catch (e) { console.error("[god.tui/commands/authdir.ts] " + ((e as any)?.message || e)); return { enabled: [], disabled: [] }; }
}
...
async function manifestToolNames(): Promise<string[]> {
  ...
  } catch (e) { console.error("[god.tui/commands/authdir.ts] " + ((e as any)?.message || e)); return []; }
}
...
export async function authdirCompletions(prefix: string) {
  ...
  } catch (e) { console.error("[god.tui/commands/authdir.ts] " + ((e as any)?.message || e)); }
  return items.length ? items : null;
}
