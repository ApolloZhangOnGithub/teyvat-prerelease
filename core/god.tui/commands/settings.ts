...
function featureEntries(): any[] {
  ...
  const guard = (fn: () => any) => async () => {
    try { await fn(); } catch (e) { console.error("[god.tui/commands/settings.ts] feature entry: " + ((e as any)?.message || e)); }
  };
  return [
...
function save(key: string, value: any) {
  ...
  } catch (e) { console.error("[god.tui/commands/settings.ts] " + ((e as any)?.message || e)); }
}
...
function getAllItems() {
  ...
  const amemDisplay = g.__genshinAmemDisplay ?? "show";
  ...
  const items: any[] = [
    ...
    { id: "toolExpanded", label: T("工具输出详略", "Tool Output Detail"), currentValue: toolMode, values: [T("摘要", "Summary"), T("完整", "Full")] },
    ...
    // 2026-09-15（用户）：工具输出隐藏开关——Read/Write/Edit 结果块整体不显示（只留调用行），错误仍显示
    { id: "toolOutputShow", label: T("工具输出显示", "Tool Output Show"), currentValue: ((g as any).__genshinToolOutputHide === true) ? T("隐藏", "Hide") : T("显示", "Show"), values: [T("显示", "Show"), T("隐藏", "Hide")] },
    { id: "resultAt", label: T("at 时间戳", "At Timestamp"), currentValue: resultAt ? T("显示", "Show") : T("隐藏", "Hide"), values: [T("显示", "Show"), T("隐藏", "Hide")] },
    // 2026-09-13（用户）：Amem 输出——隐藏/显示（默认显示）；2026-09-23 去“折叠”（结果已是单行摘要，折叠与显示同效果→形同虚设）
    { id: "amemDisplay", label: T("Amem 输出", "Amem Output"), currentValue: amemDisplay === "hide" ? T("隐藏", "Hide") : T("显示", "Show"), values: [T("显示", "Show"), T("隐藏", "Hide")] },
  ];
  ...
  const ctxUpdatedAgo = g.__genshinCtxUpdatedAgo ?? false;
  ...
  items.push(
    ...
    { id: "ctxUpdatedAgo", label: T("context 更新时间", "ctx updated ago"), currentValue: ctxUpdatedAgo ? T("显示", "Show") : T("隐藏", "Hide"), values: [T("隐藏", "Hide"), T("显示", "Show")] },
...
function handleChange(id: string, value: string) {
  ...
  switch (id) {
    ...
    case "toolOutputShow": {
      // 2026-09-15（用户）：工具输出隐藏开关（Read/Write/Edit 结果块）
      const hide = value === T("隐藏", "Hide");
      g.__genshinToolOutputHide = hide;
      save("toolOutputHide", hide);
      break;
    }
    case "resultAt": {
    ...
    case "amemDisplay": {
      // 2026-09-13（用户）：Amem 输出 隐藏/显示（默认显示）
      const mode = value === T("隐藏", "Hide") ? "hide" : "show";
    ...
    case "footerBalance":
      ...
      break;
    case "ctxUpdatedAgo":
      g.__genshinCtxUpdatedAgo = value === T("显示", "Show");
      save("ctxUpdatedAgo", g.__genshinCtxUpdatedAgo);
      break;
