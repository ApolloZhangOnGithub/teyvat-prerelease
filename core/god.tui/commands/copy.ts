...
export async function copyHandler(_args: any, ctx: any) {
  try { console.error(`[copy-handler] invoked, bridge=${typeof (globalThis as any).__genshinHandleCopyCommand}`); } catch (e) { console.error("[god.tui/commands/copy.ts] " + ((e as any)?.message || e)); }
