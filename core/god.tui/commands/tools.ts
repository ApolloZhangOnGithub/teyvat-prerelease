...
function sessionOverrides(): Record<string, boolean> {
  ...
  } catch (e) { console.error("[god.tui/commands/tools.ts] " + ((e as any)?.message || e)); }
  return {};
}
...
function saveSessionOverrides(ov: Record<string, boolean>): boolean {
  try {
    ...
    return true;
  } catch (e) { console.error("[god.tui/commands/tools.ts] " + ((e as any)?.message || e)); return false; }
}
