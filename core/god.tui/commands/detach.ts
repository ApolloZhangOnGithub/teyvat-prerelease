...
export async function detachHandler(_args: any, ctx: any) {
  ...
  } catch (e) { console.error("[god.tui/commands/detach.ts] " + ((e as any)?.message || e)); }
  try {
...
export function spawnHeadlessBg(pid: string, reason: string): void {
  ...
  child.on("error", (e: any) => {
    console.error("[god.tui/commands/detach.ts] 后台 headless 启动失败: " + (e?.code || e?.message || e));
  });
