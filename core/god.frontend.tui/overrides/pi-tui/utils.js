...
// teyvat 2026-08-14（ISSUE 092）：Intl.Segmenter.segment() 每次调用创建/销毁 ICU BreakIterator → 长会话 GC 风暴。
 ...
 */
// teyvat 2026-08-15（ISSUE 092 续）：wrapTextWithAnsi 行级 memo——"窗口没变化就不从头渲染"
 ...
 */
// teyvat: 提取行尾的 ANSI SGR reset + OSC 8 close 序列（不可见序列链）
...
export function applyBackgroundToLine(line, width, bgFn) {
    ...
    // Apply background to content + padding（teyvat: 尾部不可见序列留在空格之后）
