...
export class ProcessTerminal {
    ...
    setupStdinBuffer() {
        ...
        this.stdinBuffer.on("data", (sequence) => {
            if (process.env.INPUTPS03 === "0") return; // teyvat: 输入协议调试开关（0.80.7 时代定制）
        ...
        this.stdinDataHandler = (data) => {
            if (process.env.INPUTPS02 === "0") return; // teyvat: 输入协议调试开关
    ...
    queryAndEnableKittyProtocol() {
        ...
        if (process.env.INPUTPS01 !== "0") process.stdout.write(KITTY_KEYBOARD_PROTOCOL_QUERY); // teyvat: 输入协议调试开关
    }
    ...
    forwardInputSequence(sequence) {
        if (!this.inputHandler)
            return;
        if (process.env.INPUTPS04 === "0") return; // teyvat: 输入协议调试开关
