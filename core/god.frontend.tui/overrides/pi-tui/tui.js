...
/**
 * teyvat: 渲染异常隔离 —— 组件 render() 抛错时记日志并跳过，绝不让一个坏组件
...
export function renderErrorLog(code, message) {
    ...
    try {
        const dir = path.join(os.homedir(), ".teyvat", "agent");
...
export class Container {
    ...
    render(width) {
        ...
        for (const child of this.children) {
            ...
            try {
                // teyvat 2026-08-20: child 可能非组件（字符串/原始值）——防御性渲染，字符串作单行
...
export class TuiBase extends Container {
    ...
    constructor(terminal, showHardwareCursor, logDirectory) {
        ...
        // 2026-08-18 统一配置目录（用户定稿：只用 ~/.teyvat，不用 ~/.pi）：
        // 原逻辑读 PI_CODING_AGENT_DIR（0.3.0 更名前的变量名，launcher 已弃用）且回退 ~/.pi/agent。
        // teyvat 的 agent 目录真相源 = GENSHIN_CODING_AGENT_DIR（APP_NAME=genshin 派生，launcher.sh 设置）
        // 或 PAIMON_CODING_AGENT_DIR（launcher 同时保留）——回退路径统一到 ~/.teyvat/agent。
        this.logDirectory = logDirectory ?? process.env.GENSHIN_CODING_AGENT_DIR ?? process.env.PAIMON_CODING_AGENT_DIR ?? path.join(os.homedir(), ".teyvat", "agent");
    ...
    requestRender(force = false) {
        if (process.env.PAIMON_HEADLESS === "1") return; // teyvat: headless 调试开关（0.80.7 时代定制）
        if (force) {
            this.resetRenderState();
    ...
    scheduleRender() {
        if (process.env.PAIMON_HEADLESS === "1") return; // teyvat: headless 调试开关
    ...
    handleTerminalInput(data) {
        if (process.env.INPUTPS05 === "0") return; // teyvat: 输入协议调试开关
        ...
        if (this.focusedComponent?.handleInput) {
            ...
            if (process.env.INPUTPS06 !== "0") this.focusedComponent.handleInput(data); // teyvat: 输入协议调试开关
            ...
            if (process.env.INPUTPS07 !== "0") this.requestImmediateRender(); // teyvat: 输入协议调试开关
        }
    }
