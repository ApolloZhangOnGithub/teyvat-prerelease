...
function addIgnoreRules(ig, dir, rootDir) {
    ...
    for (const filename of IGNORE_FILE_NAMES) {
        ...
        catch(e) { try { require("fs").appendFileSync((process.env.HOME||"")+"/.teyvat/LogData/genshin-catch-errors.log", "[1E001] " + (e?.stack||e) + "\n"); } catch (e) { console.error("[god.frontend.tui/overrides/pi-dist/core/package-manager.js] " + (e?.message || e)); } }
    }
}
...
function collectAutoExtensionEntries(dir) {
    ...
    try {
        ...
        for (const entry of dirEntries) {
            ...
            if (entry.name === "node_modules")
                continue;
            // teyvat: 跳过【禁用/备份】的扩展目录 —— 停用一个扩展只需就地改名,不必移出 extensions/
