    ...
    _handleAgentEvent = async (event) => {
        ...
        if (event.type === "message_end") {
            ...
            if (event.message?.customType === "memory-snapshot" && typeof event.message.content === "string" && event.message.content.startsWith("__gz__")) {
                ...
                try { event.message.content = gunzipSync(Buffer.from(event.message.content.slice(6), "base64")).toString("utf8"); } catch (e) { console.error("[god.frontend.tui/overrides/pi-dist/core/agent-session.js] " + (e?.message || e)); }
            }
            ...
            if (event.message.role === "custom") {
                ...
                if (event.message.customType === "memory-snapshot" && typeof content === "string" && content.length > 1000) {
                    ...
                    try { content = "__gz__" + gzipSync(content).toString("base64"); } catch (e) { console.error("[god.frontend.tui/overrides/pi-dist/core/agent-session.js] " + (e?.message || e)); }
                }
