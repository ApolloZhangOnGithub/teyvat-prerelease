// ═══════════════════════════════════════════════════════════════════════════
// teyvat 覆盖版（2026-08-20）：pi 原版 write.js + force 参数
// 背景：用户定稿「write 设置路由禁止模型写到 /tmp，一定要写要用 force 强制」。
//   - writeSchema 加 force:boolean（可选）——模型确需写 /tmp 时传 force:true
//   - 实际拦截在 fileacts.ts 的 tool_call gate（默认禁 /tmp，force 放行）
//   - 此覆盖只改 schema（让模型能传 force），执行逻辑与 pi 原版一致
// ═══════════════════════════════════════════════════════════════════════════
...
const writeSchema = Type.Object({
    ...
    // 2026-08-20 teyvat：写 /tmp 默认被 gate 禁止（临时文件应写 AgentWorkDir）——确需写 /tmp 时传 force:true
    force: Type.Optional(Type.Boolean({ description: "Write outside the workdir (e.g. /tmp) is blocked by default — set force:true to allow (temporary files should normally go to your AgentWorkDir)." })),
});
