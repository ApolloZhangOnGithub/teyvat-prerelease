 ...
 *
 * OVERRIDE: normalizeForFuzzyMatch now also normalizes leading whitespace
 * (tabs → spaces, ANSI strip) so model indentation drift doesn't break matching.
 */
 ...
 * - Normalize leading whitespace: tabs → 2 spaces, strip ANSI escape codes
...
export function normalizeForFuzzyMatch(text) {
    return (text
        ...
        .split("\n")
        .map((line) => {
            // Normalize leading whitespace: tabs → 2 spaces, strip ANSI
            // Then trim trailing whitespace
            let l = line.trimEnd();
            const m = l.match(/^(\s*)/);
            if (m && m[1]) {
                l = m[1].replace(/\t/g, "  ").replace(/\x1b\[[0-9;]*m/g, "") + l.slice(m[1].length);
            }
            // Blank lines → empty
            return l.trim() === "" ? "" : l;
        })
