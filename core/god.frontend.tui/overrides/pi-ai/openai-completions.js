...
// ── RESEARCH logits 记录管线（解耦模块，见 Proposals/032）────────
// 记录每个输出 token 的 top-N logits 到 /ResearchData/<agentid>/，
// gzip 流式压缩 JSONL，与 session 事件同序。
// 开关：默认开（PI_RESEARCH=off 关闭）。当前只记录 researcher 本人，
// 推广全部 agent 待用户确认（Proposals/032 实施步骤）。
...
const researchLogits = {
  ...
  writer(sessionId) {
    ...
    const dir = join(homedir(), ".teyvat", "ResearchData", aid);
    // [BUG 2026-08-15 by memory-control-developer-01] 原代码无 mkdirSync → ResearchData 目录不存在时 ENOENT 闪退
    try { mkdirSync(dir, { recursive: true }); } catch (e) { console.error("[god.frontend.tui/overrides/pi-ai/openai-completions.js] " + (e?.message || e)); }
    createWriteStream(join(dir, "index.jsonl"), { flags: "a" })
      .end(JSON.stringify({ ts: new Date().toISOString(), sessionId: key }) + "\n");
    const file = join(dir, `${key}.logits.gz`);
    const gz = createGzip();
    const out = createWriteStream(file, { flags: "a" });
    gz.pipe(out);
    w = { gz, out, file, count: 0 };
  ...
  append(sessionId, record) {
    ...
    try {
      const w = this.writer(sessionId);
      w.gz.write(JSON.stringify(record) + "\n");
      // 定期 flush：gzip 流有内部 buffer，不 flush 则运行中的数据不落盘，
      // 进程崩溃会丢 buffer 里的记录。每 50 条 flush 一次，最多丢 50 条。
      if (++w.count % 50 === 0) w.gz.flush();
    } catch { /* 静默：记录不能影响正常流程 */ }
  },
  // 进程退出/中断时 flush 所有 gzip 流，避免尾部记录丢失。
  closeAll() {
    for (const w of this._writers.values()) {
      try { w.gz.flush(); w.gz.end(); } catch { /* ignore */ }
    }
    this._writers.clear();
  },
};
...
process.once("exit", () => { try { researchLogits.closeAll(); } catch { /* ignore */ } });
process.once("SIGINT", () => { try { researchLogits.closeAll(); } catch { /* ignore */ } });
process.once("SIGTERM", () => { try { researchLogits.closeAll(); } catch { /* ignore */ } });
    ...
    (async () => {
        ...
        try {
            ...
            for await (const chunk of openaiStream) {
                ...
                // RESEARCH 管线：采集当前 chunk 的 logprobs（Proposals/032）
                if (researchLogits.enabled() && choice.logprobs) {
                    const ts = new Date().toISOString();
                    const base = { ts, sessionId: cacheSessionId, model: model.id };
                    // 过滤 -9999 占位（provider 对不在 top-N 内的 token 用 -9999 填充，存了浪费空间）
                    const cleanTop = (tops) => (tops || []).filter(t => typeof t?.logprob === "number" && t.logprob > -100).slice(0, 20);
                    for (const it of choice.logprobs.reasoning_content || []) {
                        if (typeof it?.logprob !== "number" || it.logprob <= -100) continue;
                        researchLogits.append(cacheSessionId, { ...base, segment: "reasoning", token: it.token, logprob: it.logprob, bytes: it.bytes || null, top_logprobs: cleanTop(it.top_logprobs) });
                    }
                    for (const it of choice.logprobs.content || []) {
                        if (typeof it?.logprob !== "number" || it.logprob <= -100) continue;
                        researchLogits.append(cacheSessionId, { ...base, segment: "content", token: it.token, logprob: it.logprob, bytes: it.bytes || null, top_logprobs: cleanTop(it.top_logprobs) });
                    }
                }
                if (choice.delta) {
            ...
            // 2026-09-05（ISSUE 126 排查，debug 定位）：formatProviderError 只返回 message——
            // TypeError（"reading includes"）的原始 stack 丢失，上游只有 message 无法定位。
            // 保留原始 error.stack 到 output.errorStack，heart agent_end 错误日志可记录定位真实炸点。
            try { output.errorStack = error?.stack || String(error); } catch { /* 不阻塞 */ }
            ...
            // 2026-09-04 teyvat 防御（ISSUE 126）：402 的错误结构特殊（metadata.previous_errors 数组），
            // formatProviderError 对它可能返回 undefined → errorMessage undefined → .includes 抛
            // "Cannot read properties of undefined (reading 'includes')"（uncaught，crash.log 无栈）。
            // 修：undefined 时用 rawMetadata 兜底（错误信息不丢），有值才做 includes 去重。
            if (rawMetadata) {
                if (!output.errorMessage) output.errorMessage = String(rawMetadata);
                else if (!output.errorMessage.includes(String(rawMetadata))) output.errorMessage += `\n${rawMetadata}`;
            }
...
function buildParams(model, context, options, compat = getCompat(model), cacheRetention = resolveCacheRetention(options?.cacheRetention, options?.env)) {
    ...
    // RESEARCH 管线：请求 top-N logits（Proposals/032）
    if (researchLogits.enabled()) {
    ...
    for (let i = 0; i < transformedMessages.length; i++) {
        ...
        if (msg.role === "user") {
            ...
            for (; j < transformedMessages.length && transformedMessages[j].role === "toolResult"; j++) {
                ...
                const toolResultText = hasText ? textResult : hasImages ? "(see attached image)" : ""; // genshin: 空 toolResult 不注入占位符（ISSUE 023 幻影文本）
