 ...
 *
 * 零依赖：仅用 node 内置模块。
 * 启动：genshin im --local   （launcher.sh 起 node server.mjs + 开 http://localhost:8790）
 * 前端：../../A.core/god.frontend.im/index.html（相对路径，源码/runtime 两端结构一致）
...
// 2026-09-13：单行箭头形式——check-sync-ua 门禁的 helper 识别只支持“单行 const 箭头 + 行内含 UA 字面量”，function 声明多行 return 不被识别（远处 fetch 会误报没带 UA）
const syncHeaders = (b) => ({ Authorization: `Bearer ${b.token}`, "X-Device-Id": b.deviceId, "User-Agent": SYNC_UA });
// 远端 agent 列表（30s 缓存）
...
const server = http.createServer(async (req, res) => {
  ...
  // 首页（前端在 god.frontend.im——源码与 runtime 的相对结构一致：../../A.core/god.frontend.im/）
  if (req.method === "GET" && (url.pathname === "/" || url.pathname === "/index.html")) {
    try {
      const html = readFileSync(join(__dir, "../../A.core/god.frontend.im/index.html"));
