...
if [ "$MODE" = "kill" ]; then
  ...
  if [[ "$NAME" =~ ^[0-9]+$ || "$NAME" =~ ^[0-9]+[fbao]$ || "$NAME" =~ ^[fbao][0-9]+$ ]]; then
  ...
  if [ -z "$PID" ]; then
    # 2026-09-25（房东实测 genshin k 1o）：原提示"没找到运行中的 1o"是误导——该 agent 明明在册，
    # 只是处于 offline（没有运行中的进程）。先查 group-map 判定它是否已知（在册）：
    #   在册且为 o 组 → 明确告知"已离线，无需 kill"（exit 0，这不是错误状态）
    #   不在册 → 才是真正的"没找到"
    _KNOWN=$(node --input-type=commonjs -e "
      const fs=require('fs');
      try{
        const gm=JSON.parse(fs.readFileSync(process.argv[1]+'/RuntimeCache/genshin-group-map.json','utf8'));
        const hit=Object.values(gm).find(v=>v.name===process.argv[2]);
        if(hit)console.log(hit.g==='o'?'offline':hit.g);
      }catch{}
    " "$PAIMON_HOME" "$NAME" 2>/dev/null)
    if [ "$_KNOWN" = "offline" ]; then
      echo "$(_l "$NAME 已经是离线状态（没有运行中的进程），无需 kill" "$NAME is already offline (no running process) — nothing to kill")"
      exit 0
    fi
    echo "$(_l "没找到运行中的 $NAME" "No running agent found: $NAME")"; exit 1
  fi
