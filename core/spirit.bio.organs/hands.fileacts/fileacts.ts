...
export function validateExecute(cmd: string, selfId?: string, cwd?: string): { blocked: boolean; message?: string } {
  ...
  if (/\b(?:python[23]?|bash|sh|zsh|node|perl|ruby)\s+-(?:c|e|eval)\b/.test(cmd)) return { blocked: true, message: i18n(`禁止直接执行解释器内联代码（python/bash/sh/node/perl/ruby -c/-e）。请在你的工作目录（AgentWorkDir）下创建脚本文件再运行；临时文件可在其下自建 tmp/ 子目录，勿用公共目录。`, `Inline interpreter code (python/bash/sh/node/perl/ruby -c/-e) is forbidden. Create a script file in your workdir (AgentWorkDir) and run it; for temporary files, make a tmp/ subdir there — don't use public directories.`) };
  if (/\bpython[23]?\s*<</.test(cmd) || /\bpython[23]?\s+-\s*$/.test(cmd)) return { blocked: true, message: i18n(`禁止直接执行 python 内联代码。请在你的工作目录（AgentWorkDir）下创建 .py 文件，然后用 python <文件名>.py 运行；临时文件可在其下自建 tmp/ 子目录。`, `Inline python code is forbidden. Create a .py file in your workdir (AgentWorkDir) then run it with python <filename>.py; for temporary files, make a tmp/ subdir there.`) };
