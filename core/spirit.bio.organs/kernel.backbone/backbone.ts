...
import { logerr, runtimeCacheDir, sessionDirFor } from "#paths";
    ...
    toolDef.execute = async function (...args: any[]) {
      ...
      try {
        if (result?.content?.length) {
          ...
          let cjk = 0; for (let i = 0; i < resultText.length; i++) { const c = resultText.charCodeAt(i); if ((c >= 0x3400 && c <= 0x9fff) || (c >= 0xf900 && c <= 0xfaff) || (c >= 0x3000 && c <= 0x30ff) || (c >= 0xff00 && c <= 0xffef)) cjk++; }
